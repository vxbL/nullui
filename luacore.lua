...
function Window:CreateTab(name, icon)
    ...
    function Tab:CreateButton(config)
        ...
        hit.MouseEnter:Connect(function() tw(btnFrame, FAST, { BackgroundColor3 = Color3.fromRGB(42, 42, 42) }) end)
        hit.MouseLeave:Connect(function() tw(btnFrame, FAST, { BackgroundColor3 = C.w12 }) end)
        hit.MouseButton1Click:Connect(function()
            tw(btnFrame, TweenInfo.new(0.08), { BackgroundColor3 = Color3.fromRGB(55, 55, 55) })
            task.delay(0.12, function() tw(btnFrame, MED, { BackgroundColor3 = C.w12 }) end)
            task.spawn(callback)
        end)
        updateCanvas()
        local BtnObj = {}
    ...
    function Tab:CreateSlider(config)
        ...
        local function apply(v)
            ...
            knob.Position = UDim2.new(p, -KNB / 2, 0.5, -KNB / 2)
            task.spawn(callback, v)
        end
        liveSetters[key] = apply

        stepMinus.MouseButton1Click:Connect(function() apply(cur - incr) end)
        stepPlus.MouseButton1Click:Connect(function()  apply(cur + incr) end)
        ...
        hitZone.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 then
                sliding = true; fromX(UserInputService:GetMouseLocation().X)
            elseif i.UserInputType == Enum.UserInputType.Touch then
                sliding = true; slideTouchId = i; fromX(i.Position.X)
            end
        end)
    ...
    function Tab:CreateDropdown(config)
        ...
        local function repositionPanel()
            ...
            local dropY    = absPos.Y + absSize.Y + 3
            local spaceBelow = screenH - dropY - 6
            local panelH
            if spaceBelow >= visiblePanelH or spaceBelow >= 40 then
                panelH = math.min(visiblePanelH, math.max(spaceBelow, 30))
                overlayPanel.Position = UDim2.new(0, absPos.X, 0, dropY)
            else
                panelH = math.min(visiblePanelH, absPos.Y - 6)
                panelH = math.max(panelH, 30)
                overlayPanel.Position = UDim2.new(0, absPos.X, 0, absPos.Y - panelH - 3)
            end
            local rightEdge = absPos.X + BTN_W
            if rightEdge > screenW - 4 then
                overlayPanel.Position = UDim2.new(0, screenW - BTN_W - 4, 0, overlayPanel.Position.Y.Offset)
            end
            return panelH
        end
    ...
    function Tab:CreateKeybind(config)
        ...
        ni("TextButton", {
            ...
            listenConn = UserInputService.InputBegan:Connect(function(i, gp)
                ...
                if newKey then stopListening(false); applyKeybind(newKey) end
            end)
        end)
...
saveBtn.MouseButton1Click:Connect(function()
    ...
    flashBtn(saveBtn, C.green); rebuildCfgUI()
end)
...
deleteBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(2, function() loadStatus.Text = "" end)
    flashBtn(deleteBtn, C.red); rebuildCfgUI()
end)
