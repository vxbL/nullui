...
local function switchTab(name)
    ...
    local prevFrame = prevName and tabFrames[prevName]
    local nextFrame = tabFrames[name]
    ...
    for n, b in pairs(tabBtns) do
        ...
        if lbl then
            ...
            lbl.TextColor3 = n == name and C.white or C.w75
        end
        tw(b, FAST, {
    ...
    if prevFrame and prevFrame.Visible then
        tw(prevFrame, EASE, { GroupTransparency = 1 })
        task.delay(0.18, function()
            prevFrame.Visible          = false
            prevFrame.GroupTransparency = 0
            prevFrame.Position         = UDim2.new(0, 0, 0, 0)
        end)
    end

    nextFrame.GroupTransparency = 1
    nextFrame.Position          = UDim2.new(0, isMobile and 12 or 20, 0, 0)
    nextFrame.Visible           = true

    TweenService:Create(ScrollFrame, EASE, { CanvasPosition = Vector2.new(0, 0) }):Play()

    task.delay(0.06, function()
        tw(nextFrame, MED, { GroupTransparency = 0, Position = UDim2.new(0, 0, 0, 0) })
    ...
    function Tab:CreateDropdown(config)
        ...
        local function repositionPanel()
            ...
            if spaceBelow >= visiblePanelH or spaceBelow >= 40 then
                ...
                panelH = math.min(visiblePanelH, absPos.Y - 8)
